...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Retrieve user details from session storage or local storage
    var userDetails = JSON.parse(sessionStorage.getItem('userDetails')) || JSON.parse(localStorage.getItem('userDetails'));
    if (userDetails) {
        ...
        document.getElementById('otherSkills').value = userDetails.otherSkills;
        // Check the appropriate skills checkboxes
        var skills = document.getElementsByName('skills');
        for (var i = 0; i < skills.length; i++) {
            if (skills[i].value === userDetails.skill && skills[i].type === 'checkbox') {
